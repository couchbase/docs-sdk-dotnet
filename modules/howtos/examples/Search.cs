...
internal class SearchV2Examples
{
    ...
    async Task ScopedConsistentWithSearch(IScope scope, ICluster cluster)
    {
    ...
    async Task ScopedVectorQueryMultiple(IScope scope)
    {
        // in actual usage, these vectors would be generated by some other AI toolkit.
        float[] preGeneratedVectors = new[] { 0.001f, 0.002f, 0.003f };
        var vectorQuery = new VectorQuery("vector-field-1").WithVector(preGeneratedVectors);
        var anotherVectorQuery = new VectorQuery("vector-field-2").WithVector(preGeneratedVectors);
        //tag::scopedVector2[]
        var searchRequest = SearchRequest.Create(
