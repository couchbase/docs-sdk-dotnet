...
:reftext: Distributed ACID Transactions from the .NET SDK
...
The starting point is the `Transactions` object, which is a property of the `Cluster`.
...
// TODO: need an example of some async transaction things (like multiple get/replace 
//   operations) to show what we mean by the above.
