...
There is a special .NET Core style Dependency Injection (DI) framework for Cluster and Buckets. It simplifies cluster configuration, lifetime management, and bucket injection. You can find it on NuGet.org: https://www.nuget.org/packages/Couchbase.Extensions.DependencyInjection/[NuGet package^]. Or using the NuGet Package Manager, add the dependency directly to your project:
...
[source,c#]
----
include::WebStartupDI-IMyBucketProvider.cs[tag=configureservices,indent=0]
----
...
[source,c#]
----
include::example$HomeControllerDI.cs[tag=mybucketprovider,indent=0]
----
