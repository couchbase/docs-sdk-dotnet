...
This default `ClusterEnvironment` is managed completely by the .NET SDK, and is automatically shut down when the associated `Cluster` is disconnected.
...
A connection string may optionally be prefixed by either `"couchbase://"` or `"couchbases://"`, although the .NET SDK completely ignores this prefix if present.
...
////
Delete this, but add in any useful .NET-specific info
== Async and Reactive APIs

The Couchbase .NET SDK provides first-class support for asynchronous and reactive programming.
In fact, the synchronous API is just a thin wrapper around the asynchronous API.

Methods in the asynchronous API return instances of the standard `CompletableFuture` introduced in .NET 8.

The .NET SDK's reactive API is built on link:https://projectreactor.io[Project Reactor].
Reactor implements the standard Reactive Streams API introduced in .NET 9, and extends it with a rich set of useful operators.
