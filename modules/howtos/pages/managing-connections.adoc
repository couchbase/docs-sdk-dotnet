...
:page-toclevels: 2
:api-bucket-link: https://docs.couchbase.com/sdk-api/couchbase-net-client/api/Couchbase.IBucket.html#Couchbase_IBucket_WaitUntilReadyAsync_TimeSpan_Couchbase_Diagnostics_WaitUntilReadyOptions_
...
[source,csharp]
----
include::example$ManagingConnections.cs[tag=network-and-timeout,indent=0]
...
(but see the *IMPORTANT* notice, above, if you are running SDK 3.7.1 or earlier).
...
If you are working at the _Bucket_ level, then the {api-bucket-link}[Bucket-level `waitUntilReady`] does the same as the Cluster-level version,
