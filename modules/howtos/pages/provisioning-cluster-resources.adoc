...
Note that you require at least the https://docs.couchbase.com/server/{version-server}/learn/security/roles.html#bucket-admin[Bucket Admin] privilege to create or drop a Scope.
The same role also lets you manage Collections, though a more minimal set of privileges is https://docs.couchbase.com/server/{version-server}/learn/security/roles.html#manage-collections-in-scope[Manage Collections in Scope] along with https://docs.couchbase.com/server/{version-server}/learn/security/roles.html#data-reader[Data Reader].

You can create users with the appropriate RBAC programmatically:

[source,csharp]
----
include::example$Couchbase.Examples/CollectionManager.cs[tag=bucketAdmin, indent=0]
