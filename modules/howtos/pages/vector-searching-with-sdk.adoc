...
[source,csharp]
----
include::example$Search.cs[tag=scopedVector1,indent=0]
----
...
Finally we execute the `SearchRequest` against the Search index "travel-vector-index", which has previously been setup to vector index the "vector_field" field.
...
* Vector GSI Query is just like any other query from the SDK to the Query service: https://docs.couchbase.com/sdk-api/couchbase-net-client/api/Couchbase.Search.Queries.Vector.VectorQuery.html[.NET SDK API Reference].
...
* Vector Search in the https://docs.couchbase.com/sdk-api/couchbase-net-client/api/Couchbase.Search.Queries.Vector.VectorSearch.html[.NET API reference].
