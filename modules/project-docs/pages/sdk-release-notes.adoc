...
For release notes, download links, and installation methods for 2.7 and earlier releases of the Couchbase .NET Client,
please see the https://docs-archive.couchbase.com/dotnet-sdk/2.7/sdk-release-notes.html[2.x .NET Release Notes & Download Archive].
...
=== Version 3.9.6 (03 September 2026)

https://packages.couchbase.com/clients/net/3.9/Couchbase-Net-Client-3.9.6.zip[Download] |
https://docs.couchbase.com/sdk-api/couchbase-net-client-3.9.6[API Reference] |
https://www.nuget.org/packages/CouchbaseNetClient/3.9.6[Nuget]


==== Fixed Issues

* https://couchbasecloud.atlassian.net/browse/NCBC-4287[NCBC-4287]:
Key-value operations are now framed from what the connection negotiated rather than from client-side state;
a mismatch could store a document under an ID that no longer mapped to its own location, or address the wrong collection.
This includes `GetAllReplicasAsync`, which could dispatch a key before resolving the collection ID (https://couchbasecloud.atlassian.net/browse/NCBC-4285[NCBC-4285]).
A connection whose `HELO` fails is now rejected instead of joining the pool reporting no server features.

* https://couchbasecloud.atlassian.net/browse/NCBC-4291[NCBC-4291]:
Lost-transaction cleanup no longer stops for one `Cluster` when another `Cluster` registered against the same keyspace is disposed;
cleanup registrations are now owned per `Cluster` rather than shared across the process.

* https://couchbasecloud.atlassian.net/browse/NCBC-4280[NCBC-4280]:
A node is no longer discarded when one of its key-value connections fails to connect but another succeeds,
which could leave the SDK with no usable nodes and report `Cluster has not yet bootstrapped` until the operation timed out;
a failed bootstrap attempt is also no longer reported as a success carrying no inner exceptions.
...
A retryable error arriving part-way through a Stellar query or analytics result stream now surfaces as `RequestCanceledException` with its error context,
...
Partially addresses unbounded internal waits:
cluster version detection now honours a `CancellationToken` through to the underlying HTTP request,
and the transaction ATR-initialisation lock wait is now bounded by the attempt's remaining time so a stuck holder on another code path cannot block it indefinitely.
Cancellation support is offered through a new `IClusterVersionProviderCancellable` interface,
discovered via a `GetVersionAsync` extension method, so external `IClusterVersionProvider` implementations remain binary compatible.


==== Improvements

* https://couchbasecloud.atlassian.net/browse/NCBC-4269[NCBC-4269]:
`WaitUntilReady` is now implemented for Stellar (`couchbase2` protocol) on both `ICluster` and `IBucket`, polling the gRPC health check until the server reports serving;
it previously threw `FeatureNotAvailableException`.
...
Stellar (`couchbase2` protocol) connections now set the gRPC maximum receive message size required by the specification.


=== Version 3.9.4 (16 July 2026)

https://packages.couchbase.com/clients/net/3.9/Couchbase-Net-Client-3.9.4.zip[Download] |
https://docs.couchbase.com/sdk-api/couchbase-net-client-3.9.4[API Reference] |
https://www.nuget.org/packages/CouchbaseNetClient/3.9.4[Nuget]


==== Fixed Issues

* https://couchbasecloud.atlassian.net/browse/NCBC-4243[NCBC-4243]:
Fixed corruption when reading `byte[]` values from sub-documents. `PersistentQueue<byte[]>`, `PersistentList<byte[]>`, and `LookupIn(...).ContentAs<byte[]>()` returned the raw JSON fragment instead of the decoded bytes (a regression affecting 3.8.1+ and 3.9.0+).

* https://couchbasecloud.atlassian.net/browse/NCBC-4244[NCBC-4244]:
Extended the `byte[]` sub-document read fix to Protostellar (`couchbase2` protocol),
and fixed `LookupIn`/`MutateIn` over Protostellar to honor a per-operation `Transcoder` instead of always using the cluster default.

* https://couchbasecloud.atlassian.net/browse/NCBC-4176[NCBC-4176]:
Corrected several KV sub-document behaviours to match the cross-SDK spec:
`LookupInAllReplicas` now returns an empty stream on a top-level failure, a `Get` with projections omits non-existent paths (including nested ones) rather than emitting nulls, and a sub-document counter overflow now surfaces `ValueInvalidException`.

* https://couchbasecloud.atlassian.net/browse/NCBC-4036[NCBC-4036]:
Fixed the transactional query error "cause" so it deserializes correctly under the default serializer;
a retryable CAS mismatch at commit is now retried instead of being surfaced as a non-retryable `TransactionFailedException`.

* https://couchbasecloud.atlassian.net/browse/NCBC-4240[NCBC-4240]:
TLS trust now loads the full certificate chain from a multi-certificate PEM (for example a cluster CA plus root CA),
fixing `CustomRootTrust` handshake failures where only the first certificate was previously loaded.

* https://couchbasecloud.atlassian.net/browse/NCBC-4214[NCBC-4214]:
`StellarCluster` now disposes the HTTP handler it creates, fixing a leak of the handler and its keep-alive timers when the cluster is disposed.

* https://couchbasecloud.atlassian.net/browse/NCBC-4242[NCBC-4242]:
On .NET Framework, the per-ATR transaction cleanup time budget is now computed with sub-millisecond precision instead of being rounded to the nearest millisecond.

* https://couchbasecloud.atlassian.net/browse/NCBC-4226[NCBC-4226]:
Analytics link and dataverse identifiers now escape embedded backticks, preventing {sqlpp} identifier injection.


==== Improvements

* https://couchbasecloud.atlassian.net/browse/NCBC-4100[NCBC-4100]:
SASL authentication now negotiates the strongest mechanism the server and client both support, and defaults to stronger SCRAM (SHA-256/SHA-512) instead of SCRAM-SHA1, aligning with NIST SP 800-131A. 
(netstandard targets remain SHA-1 due to platform limitations.)

* https://couchbasecloud.atlassian.net/browse/NCBC-4218[NCBC-4218]:
Applications that do not use transactions no longer eagerly start the transaction cleanup machinery, avoiding a parked thread-pool thread and the resulting latency spikes (for example on `Ping`) in CPU-constrained environments such as Kubernetes pods.

* https://couchbasecloud.atlassian.net/browse/NCBC-4009[NCBC-4009]:
Lost-transaction cleanup now processes ATRs in adaptive batches, so it keeps pace under CPU pressure and stays within its configured window.

* https://couchbasecloud.atlassian.net/browse/NCBC-4219[NCBC-4219]:
Lost-transaction cleanup now resolves its collections lazily, per cleanup cycle, instead of resolving them eagerly (via sync-over-async) at construction.


=== Version 3.9.3 (03 June 2026)

https://packages.couchbase.com/clients/net/3.9/Couchbase-Net-Client-3.9.3.zip[Download] |
https://docs.couchbase.com/sdk-api/couchbase-net-client-3.9.3[API Reference] |
https://www.nuget.org/packages/CouchbaseNetClient/3.9.3[Nuget]


==== Fixed Issues

* https://couchbasecloud.atlassian.net/browse/NCBC-4038[NCBC-4038]:
Query, Analytics, and Search requests that hit a node going offline during a rebalance now fail over to a healthy node within the request timeout, instead of surfacing a `RequestCanceledException` to the caller.

* https://couchbasecloud.atlassian.net/browse/NCBC-4205[NCBC-4205]:
...
* https://couchbasecloud.atlassian.net/browse/NCBC-4209[NCBC-4209]:
`ICluster` now implements `IClusterAuthenticator`, so `cluster.Authenticator()` can be called without first casting to the concrete `Cluster` type.
...
https://docs.couchbase.com/sdk-api/couchbase-net-client-3.9.1[API Reference] |
https://www.nuget.org/packages/CouchbaseNetClient/3.9.1[Nuget]


==== Fixed Issues

* https://couchbasecloud.atlassian.net/browse/NCBC-4192[NCBC-4192]:
Resolved `GetErrorMap` error while bootstrapping.

* https://couchbasecloud.atlassian.net/browse/NCBC-4139[NCBC-4139]:
Implemented tracing spans and metrics for the Management API.

* https://couchbasecloud.atlassian.net/browse/NCBC-4138[NCBC-4138]:
Resolved several tracing issues to match the updated spec.

* https://couchbasecloud.atlassian.net/browse/NCBC-4180[NCBC-4180]:
Proper staging of expiry on mutations.

* https://couchbasecloud.atlassian.net/browse/NCBC-4160[NCBC-4160]:
Fixed an issue where AppTelemetry reports significantly lower metrics than it should.

* https://couchbasecloud.atlassian.net/browse/NCBC-4153[NCBC-4153]:
Fixed NullReference errors during rebalance.

* https://couchbasecloud.atlassian.net/browse/NCBC-4151[NCBC-4151]:
Ensured collection support is negotiated earlier in the connection process.

* https://couchbasecloud.atlassian.net/browse/NCBC-4146[NCBC-4146]:
Fixed an issue where a possible null byte was prepended to keys.

* https://couchbasecloud.atlassian.net/browse/NCBC-4127[NCBC-4127]:
Resolved Transaction issues with older (7.2) servers.


==== Improvements

* https://couchbasecloud.atlassian.net/browse/NCBC-4183[NCBC-4183]:
Ensured correctness of recorded metrics and improved performance of AppTelemetry.

* https://couchbasecloud.atlassian.net/browse/NCBC-4165[NCBC-4165]:
Added Snappy compression support in Stellar (also addresses NCBC-4155 and NCBC-4156).

* https://couchbasecloud.atlassian.net/browse/NCBC-4181[NCBC-4181]:
Updated packages for vulnerable `Microsoft.Bcl.Memory`.

* https://couchbasecloud.atlassian.net/browse/NCBC-4140[NCBC-4140]:
Implemented Span Status and retries for tracing.


=== Version 3.9.0 (26 February 2026)

https://packages.couchbase.com/clients/net/3.9/Couchbase-Net-Client-3.9.0.zip[Download] |
https://docs.couchbase.com/sdk-api/couchbase-net-client-3.9.0[API Reference] |
https://www.nuget.org/packages/CouchbaseNetClient/3.9.0[Nuget]


==== Fixed Issues

* https://couchbasecloud.atlassian.net/browse/NCBC-4143[NCBC-4143]:
Fixed an assertion issue in `MultiplexingConnection.CloseAsync`.

* https://couchbasecloud.atlassian.net/browse/NCBC-4135[NCBC-4135]:
`HttpMessageHandler` is now correctly recreated when the instance changes, regardless of authenticator type.

* https://couchbasecloud.atlassian.net/browse/NCBC-4128[NCBC-4128]:
Clear dead nodes after a select bucket failure.

* https://couchbasecloud.atlassian.net/browse/NCBC-4115[NCBC-4115]:
Resolved `NullReferenceExceptions` in some cases when calling `PingAsync`.

* https://couchbasecloud.atlassian.net/browse/NCBC-4120[NCBC-4120]:
Prevented Capella CA certificate disposal on .NET 10.

* https://couchbasecloud.atlassian.net/browse/NCBC-4104[NCBC-4104]:
Fixed `DateTime` parsing for Certificate Expiration Date.

* https://couchbasecloud.atlassian.net/browse/NCBC-4094[NCBC-4094]:
Operations that receive `BucketNotConnected` are now automatically retried.

* https://couchbasecloud.atlassian.net/browse/NCBC-4055[NCBC-4055]:
Fixed various Lock, Unlock, and Touch issues in Stellar.

* https://couchbasecloud.atlassian.net/browse/NCBC-4061[NCBC-4061]:
Fixed `QueryIndexManager` failures in Stellar.

* https://couchbasecloud.atlassian.net/browse/NCBC-4054[NCBC-4054]:
Fixed hardcoded Stellar gateway port.


==== New Features

* **Protostellar**: This release introduces full support for Protostellar (the Cloud Native Gateway protocol).
Protostellar provides a high-performance, gRPC-based client for Couchbase Server via CNG, offering improved scalability and simplified connectivity in cloud-native environments.

* https://couchbasecloud.atlassian.net/browse/NCBC-4041[NCBC-4041]:
Added support for .NET 10 LTS.

* https://couchbasecloud.atlassian.net/browse/NCBC-4098[NCBC-4098]:
Updated `OpenTelemetry` tags to align with the latest semantic conventions.

* https://couchbasecloud.atlassian.net/browse/NCBC-4112[NCBC-4112]:
Implemented JWT (JSON Web Token) Authentication.

* https://couchbasecloud.atlassian.net/browse/NCBC-4116[NCBC-4116]:
Support added for searching by cancellation of `ConnectAsync`.

* https://couchbasecloud.atlassian.net/browse/NCBC-4079[NCBC-4079]:
Initial support for Expiry in the Transactions API.

* https://couchbasecloud.atlassian.net/browse/NCBC-4060[NCBC-4060]:
Support for mTLS certificate refresh.

* https://couchbasecloud.atlassian.net/browse/NCBC-4023[NCBC-4023]:
Support added for Transcoders in Stellar.

* https://couchbasecloud.atlassian.net/browse/NCBC-3996[NCBC-3996]:
Added support for FTS-like pre-filters while performing Vector Search.


==== Improvements

* https://couchbasecloud.atlassian.net/browse/NCBC-4102[NCBC-4102]:
Refactored the SDK to use `Authenticators` for improved security handling.

* https://couchbasecloud.atlassian.net/browse/NCBC-4121[NCBC-4121]:
Upgraded all Microsoft dependencies to the latest .NET 10 versions.

* https://couchbasecloud.atlassian.net/browse/NCBC-4119[NCBC-4119]:
Upgraded all 3rd party dependencies to the latest versions.

* https://couchbasecloud.atlassian.net/browse/NCBC-4088[NCBC-4088]:
Reworked mTLS certificate refresh implementation to align closer with the RFC.

* https://couchbasecloud.atlassian.net/browse/NCBC-4090[NCBC-4090]:
Initialized `AppTelemetryCollector` after bootstrapping.

* https://couchbasecloud.atlassian.net/browse/NCBC-4078[NCBC-4078]:
Improved `BucketManager` error handling for null bucket names.

* https://couchbasecloud.atlassian.net/browse/NCBC-3960[NCBC-3960]:
Added support for `accessDeleted` reads for subdocument replica reads.


==== Deprecations

* https://couchbasecloud.atlassian.net/browse/NCBC-4096[NCBC-4096]:
Deprecated support for MapReduce Views in the SDK.





== .NET SDK 3.8 Releases


=== Version 3.8.1 (10 November 2025)

https://packages.couchbase.com/clients/net/3.8/Couchbase-Net-Client-3.8.1.zip[Download] |
https://docs.couchbase.com/sdk-api/couchbase-net-client-3.8.1[API Reference] |
https://www.nuget.org/packages/CouchbaseNetClient/3.8.1[Nuget]

==== Fixed Issues

* https://couchbasecloud.atlassian.net/browse/NCBC-3898[NCBC-3898]:
`Cluster.Dispose` now safely ensures disposed `ChannelConnectionPool` objects cannot be re-used.

* https://couchbasecloud.atlassian.net/browse/NCBC-4053[NCBC-4053]:
`accessedDeleted` should now be sent when using `ReadPreference`.

* https://couchbasecloud.atlassian.net/browse/NCBC-4078[NCBC-4078]:
`BucketManager.GetBucketAsync` now throws an `ArgumentNullException` instead of `JsonException` if `bucketName` is null.


==== New Feature

* https://couchbasecloud.atlassian.net/browse/NCBC-3996[NCBC-3996]: 
Added support for FTS Pre-Filters using Vector Search.


=== Version 3.8.0 (20 August 2025)

https://packages.couchbase.com/clients/net/3.8/Couchbase-Net-Client-3.8.0.zip[Download] |
https://docs.couchbase.com/sdk-api/couchbase-net-client-3.8.0[API Reference] |
https://www.nuget.org/packages/CouchbaseNetClient/3.8.0[Nuget]


==== Fixed Issues

* https://couchbasecloud.atlassian.net/browse/NCBC-3900[NCBC-3900]:
Added logic to ensure a disposed `ChannelConnectionPool` is not reused.

* https://jira.issues.couchbase.com/browse/NCBC-4032[NCBC-4032]:
Fixed serialization issues in Transactions when mixing `System.Text.Json` and `Newtonsoft.Json`.

==== New Features

* https://couchbasecloud.atlassian.net/browse/NCBC-3961[NCBC-3961]:
`BucketSettings` now supports VBucket number configuration.

* https://couchbasecloud.atlassian.net/browse/NCBC-3998[NCBC-3998]:
Added support for Zone-Aware read from replica to Collection API.

* https://jira.issues.couchbase.com/browse/NCBC-3999[NCBC-3999]:
Added `GetReplicaFromPreferredServerGroup` to Transactions API.

* https://jira.issues.couchbase.com/browse/NCBC-3956[NCBC-3956]:
Added support for Enhanced Read Committed Isolation to Transactions API.

* https://jira.issues.couchbase.com/browse/NCBC-3778[NCBC-3778]:
Added support for Binary objects in Transactions.

* https://couchbasecloud.atlassian.net/browse/NCBC-4018[NCBC-4018]:
Stellar client: Implemented Binary Collection KV API.

* https://couchbasecloud.atlassian.net/browse/NCBC-3896[NCBC-3896]:
Added SDK telemetry collection in Server.

==== Improvements

* https://couchbasecloud.atlassian.net/browse/NCBC-3846[NCBC-3846]:
Sealed internal classes for devirtualization performance gains.

* https://couchbasecloud.atlassian.net/browse/NCBC-3847[NCBC-3847]:
Optimized reading of operation response lengths.

* https://couchbasecloud.atlassian.net/browse/NCBC-3848[NCBC-3848]:
Used bit blitting to improve performance when reading response headers.

* https://couchbasecloud.atlassian.net/browse/NCBC-3861[NCBC-3861]:
Improved `CancellationTokenPairSource` performance using pooling.

* https://couchbasecloud.atlassian.net/browse/NCBC-3904[NCBC-3904]:
Improved `OperationBuilder` performance using `IBufferWriter<byte>`.

* https://couchbasecloud.atlassian.net/browse/NCBC-3905[NCBC-3905]:
Used `TimeProvider` to improve unit test run times.

* https://couchbasecloud.atlassian.net/browse/NCBC-3908[NCBC-3908]:
Documentation no longer shows `RequestPlus` being used as a `SearchOptions` value.

* https://couchbasecloud.atlassian.net/browse/NCBC-3962[NCBC-3962]:
Improved SDK error messages for account lock/unlock.

* https://couchbasecloud.atlassian.net/browse/NCBC-4034[NCBC-4034]:
The operational SDK now prevents querying the Analytics service from Enterprise Analytics clusters.
...
https://packages.couchbase.com/clients/net/3.7/Couchbase-Net-Client-3.7.2.zip[Download] |
https://docs.couchbase.com/sdk-api/couchbase-net-client-3.7.2[API Reference] |
https://www.nuget.org/packages/CouchbaseNetClient/3.7.2[Nuget]


==== Fixed Issues

* https://couchbasecloud.atlassian.net/browse/NCBC-4027[NCBC-4027]:
`ForceIpAsTargetHost` is now `false` by default.
This is in line with most production needs, and will prevent hostname verification failing (since TLS certificates are tied to the hostname).  

* https://couchbasecloud.atlassian.net/browse/NCBC-3715[NCBC-3715]:
For Cloud Native Gateway [CNG]: Subdoc Mutation Array now handles `removeBrackets` bool properly.

* https://couchbasecloud.atlassian.net/browse/NCBC-4017[NCBC-4017]:
`ReplaceAsync` CAS field is now properly set on the operation.

==== Improvements

* https://couchbasecloud.atlassian.net/browse/NCBC-4026[NCBC-4026]:
Upgraded `Grpc.Net.Client` and other related `Grpc` dependencies,
owing to https://nvd.nist.gov/vuln/detail/CVE-2023-32731[CVE-2023-32731] (https://osv.dev/vulnerability/GHSA-cfgp-2977-2fmm[GHSA-cfgp-2977-2fmm])

* https://couchbasecloud.atlassian.net/browse/NCBC-4008[NCBC-4008]:
CNG: Some new `BucketSettings` fields have been added to the request.

* https://couchbasecloud.atlassian.net/browse/NCBC-4016[NCBC-4016]:
CNG: Passing `null` content for `Insert`, `Replace`, or `Upsert` now throws `InvalidArgumentException`.

* https://couchbasecloud.atlassian.net/browse/NCBC-4025[NCBC-4025]:
`Append` and `Prepend` now write the CAS field to the operation.


=== Version 3.7.1 (7 May 2025)

https://packages.couchbase.com/clients/net/3.7/Couchbase-Net-Client-3.7.1.zip[Download] |
https://docs.couchbase.com/sdk-api/couchbase-net-client-3.7.1[API Reference] |
https://www.nuget.org/packages/CouchbaseNetClient/3.7.1[Nuget]

==== Fixed Issues

* https://couchbasecloud.atlassian.net/browse/NCBC-4002[NCBC-4002]:
  Fixed an issue specific to .NET Framework when streaming HTTP results larger than 2GB, causing an OOM error (`Cannot write more bytes to the buffer than the configured maximum buffer size`).

==== Improvements

* https://couchbasecloud.atlassian.net/browse/NCBC-3992[NCBC-3992]:
  `KvIgnoreRemoteCertificateNameMismatch = true` was clearing other `RemoteCertificateChainErrors`.
  Further validation has been added, and this no longer occurs.
+  
IMPORTANT: For use with SSL and certificates which contain the hostname (versus an IP address), with SDK 3.7.1, set `ForceIpAsTargetHost = false` as a `ClusterOption`
(this is the default setting from release 3.7.2).

* https://couchbasecloud.atlassian.net/browse/NCBC-4006[NCBC-4006], https://couchbasecloud.atlassian.net/browse/NCBC-4010[NCBC-4010]:
  Improvements on error handling in the Transactions API.

==== New Features

* https://couchbasecloud.atlassian.net/browse/NCBC-4005[NCBC-4005]:
  Implemented `ExtParallelUnstaging` in the integrated Transactions API.
...
https://packages.couchbase.com/clients/net/3.6/Couchbase-Net-Client-3.6.6.zip[Download] |
https://docs.couchbase.com/sdk-api/couchbase-net-client-3.6.6[API Reference] |
https://www.nuget.org/packages/CouchbaseNetClient/3.6.6[Nuget]


==== Fixed Issues

* https://couchbasecloud.atlassian.net/browse/NCBC-3885[NCBC-3885]:
System certificate store should continue to be used in default TLS/SSL configuration.

* https://jira.issues.couchbase.com/browse/NCBC-3964[NCBC-3964]: Fixed issue where queries were not evenly distributed amongst Query Nodes.

==== Improvements

* https://couchbasecloud.atlassian.net/browse/NCBC-3945[NCBC-3945]:
Make `ScopeSpec` and `CollectionSpec` instance equality determinable.

* https://couchbasecloud.atlassian.net/browse/NCBC-3948[NCBC-3948]:
Add new `TryGetRawParameter` to `ClusterOptions`.


=== Version 3.6.5 (17 January 2025)

https://packages.couchbase.com/clients/net/3.6/Couchbase-Net-Client-3.6.5.zip[Download] |
https://docs.couchbase.com/sdk-api/couchbase-net-client-3.6.5[API Reference] |
https://www.nuget.org/packages/CouchbaseNetClient/3.6.5[Nuget]


==== Fixed Issues

* https://couchbasecloud.atlassian.net/browse/NCBC-3906[NCBC-3906]:
A missing `ConfigureAwait False` in `AttemptContext` was causing a build error.
This has now been fixed.

==== Improvements

* https://couchbasecloud.atlassian.net/browse/NCBC-3920[NCBC-3920]:
Renamed `Couchbase.Integrated.Transactions` to `Couchbase.Client.Transactions`.

* https://couchbasecloud.atlassian.net/browse/NCBC-3919[NCBC-3919]:
A final refactor of the transactions integration to match the initial commit for `ExtThreadSafety`, `ExtSdkIntegration`, and others.

* https://couchbasecloud.atlassian.net/browse/NCBC-3916[NCBC-3916]:
`ExtThreadSafety` implementation into integrated transactions library.

* https://couchbasecloud.atlassian.net/browse/NCBC-3915[NCBC-3915]:
Allow `TransactionConfig` to configured via `ClusterOptions`.

* https://couchbasecloud.atlassian.net/browse/NCBC-3913[NCBC-3913]:
Improvements and bug fixes to the `Cleaner` for transactions.

* https://couchbasecloud.atlassian.net/browse/NCBC-3912[NCBC-3912]:
Renamed parameter to Error Constructor from `err` to `cause`.

* https://couchbasecloud.atlassian.net/browse/NCBC-3897[NCBC-3897]:
Added `PreferReturns` support for `LookupIn`, for internal use, to improve subdoc performance for transactions.

* https://couchbasecloud.atlassian.net/browse/NCBC-3845[NCBC-3845]:
Added `ReadOnlySequence` and `IBufferWriter` support to `ITypeSerializer`.


=== Version 3.6.4 (8 November 2024)

https://packages.couchbase.com/clients/net/3.6/Couchbase-Net-Client-3.6.4.zip[Download] |
https://docs.couchbase.com/sdk-api/couchbase-net-client-3.6.4[API Reference] |
https://www.nuget.org/packages/CouchbaseNetClient/3.6.4[Nuget]


==== Fixed Issues

https://couchbasecloud.atlassian.net/browse/NCBC-3840[NCBC-3840]:
[Rider IDE] `:nq` is not a valid format specifier notice now correctly uses Debugger Display reference.

https://couchbasecloud.atlassian.net/browse/NCBC-3878[NCBC-3878]:
Randomized seed order of hosts enabled in the Connection string
+
NOTE: Random seed order must be enabled with `?random_seed_nodes=true` in the connection string.
This will be the default in a future release.

https://couchbasecloud.atlassian.net/browse/NCBC-3879[NCBC-3879]:
.NET SDK was setting span attributes directly on the operation's span, rather than on the despatch span.
Attributes have now been added to the `DispatchSpan` for Query and KV operations,
but not removed from the operation's span in order not to break user queries that target those specific traces.
+
*DEPRECATION NOTICE*: In 3.7.0, these attributes should be removed from the operation's span.

https://couchbasecloud.atlassian.net/browse/NCBC-3883[NCBC-3883]:
Updated `System.Text.Json` dependency, owing to a security vulnerability.


==== Improvements

https://couchbasecloud.atlassian.net/browse/NCBC-3851[NCBC-3851]:
More Contextual labels have been added to metrics, as part of improvements to the observability spec.
...
Some NuGet packages were upgraded as a result of high impact security issues, https://jira.issues.couchbase.com/browse/NCBC-3839[NCBC-3839].
These should not impact any users of CouchbaseNetClient, as they are backward compatible updates.
However, if you find an incompatibility, you can add a direct reference to the old version of those packages as a dependency in your own project.

https://packages.couchbase.com/clients/net/3.6/Couchbase-Net-Client-3.6.3.zip[Download] |
https://docs.couchbase.com/sdk-api/couchbase-net-client-3.6.3[API Reference] |
https://www.nuget.org/packages/CouchbaseNetClient/3.6.3[Nuget]


==== Fixed Issues

* https://jira.issues.couchbase.com/browse/NCBC-3863[NCBC-3863]:
Performance problems and timeouts fixes during rebalance when the cluster contains non-KV nodes.

* https://jira.issues.couchbase.com/browse/NCBC-3832[NCBC-3832]:
Query timeout set to exceed 100 seconds is now respected.

* https://jira.issues.couchbase.com/browse/NCBC-3818[NCBC-3818]:
`QueryErrorContext` `Statement` parameter is now set properly.

* https://jira.issues.couchbase.com/browse/NCBC-3842[NCBC-3842]:
Newtonsoft deserializing UTF8 surrogate pairs fixed for exceptions and buffer overflow.

* https://jira.issues.couchbase.com/browse/NCBC-3849[NCBC-3849]:
Orphaned responses now sent to metrics for untraced operations.

* https://jira.issues.couchbase.com/browse/NCBC-3850[NCBC-3850]:
Timeouts now recorded to metrics in `ClusterNode.ExecuteOp`.

* https://jira.issues.couchbase.com/browse/NCBC-3852[NCBC-3852]:
`TryReadMutationToken` header length check.

* https://jira.issues.couchbase.com/browse/NCBC-3854[NCBC-3854]:
Improvements made for `LightweightStopwatch` unit test.

* https://jira.issues.couchbase.com/browse/NCBC-3876[NCBC-3876]:
`CancellationTokenSource` is not disposed when returned to pool on legacy frameworks.

==== Improvements

* https://jira.issues.couchbase.com/browse/NCBC-3839[NCBC-3839]:
Updated packages based on BlackDuck scan.

* https://jira.issues.couchbase.com/browse/NCBC-3710[NCBC-3710]:
Included operation outcome tag in operation metrics.

* https://jira.issues.couchbase.com/browse/NCBC-3837[NCBC-3837]:
Provide easy configuration to exclude duplicative metrics from `OpenTelemetry`.


=== Version 3.6.2 (22 July 2024)

Version 3.6.2 is a hotfix release.
...
Support added for base64 encoded vector types.
...
* https://issues.couchbase.com/browse/NCBC-3817[NCBC-3817]:
Upgrade `System.Text.Json` to patched version.

This release also backports fixes brought to `ConfigPushHandler` in releases 3.6.0 and 3.6.1:

* https://issues.couchbase.com/browse/NCBC-3811[NCBC-3811]:
Backports ConfigPushHandler fixes to 3.5 branch
...
Distributed ACID Transactions, now incorporated within the .NET SDK, was previously available as a separate library.
Release notes for the legacy transactions library can be found in the https://docs-archive.couchbase.com/dotnet-sdk/3.3/project-docs/distributed-transactions-dotnet-release-notes.html[docs archive].
// end::all[]
