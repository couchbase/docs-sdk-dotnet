...
Extended the `byte[]` sub-document read fix to Stellar (couchbase2), and fixed `LookupIn`/`MutateIn` over Stellar to honor a per-operation `Transcoder` instead of always using the cluster default.

* https://couchbasecloud.atlassian.net/browse/NCBC-4176[NCBC-4176]:
Corrected several KV sub-document behaviours to match the cross-SDK spec: `LookupInAllReplicas` now returns an empty stream on a top-level failure, a `Get` with projections omits non-existent paths (including nested ones) rather than emitting nulls, and a sub-document counter overflow now surfaces `ValueInvalidException`.

* https://couchbasecloud.atlassian.net/browse/NCBC-4036[NCBC-4036]:
Fixed the transactional query error "cause" so it deserializes correctly under the default serializer; a retryable CAS mismatch at commit is now retried instead of being surfaced as a non-retryable `TransactionFailedException`.

* https://couchbasecloud.atlassian.net/browse/NCBC-4240[NCBC-4240]:
TLS trust now loads the full certificate chain from a multi-certificate PEM (for example a cluster CA plus root CA), fixing `CustomRootTrust` handshake failures where only the first certificate was previously loaded.
...
Analytics link and dataverse identifiers now escape embedded backticks, preventing SQL++ identifier injection.


==== Improvements

* https://couchbasecloud.atlassian.net/browse/NCBC-4100[NCBC-4100]:
SASL authentication now negotiates the strongest mechanism the server and client both support, and defaults to stronger SCRAM (SHA-256/SHA-512) instead of SCRAM-SHA1, aligning with NIST SP 800-131A. (netstandard targets remain SHA-1 due to platform limitations.)
