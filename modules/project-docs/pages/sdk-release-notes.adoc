...
=== Special Note
* During a rebalance upgrade from 6.x (or any earlier version) to 7x, in a mixed mode there is a known issue where data may be written to the wrong location.  Solution would be to either upgrade to 3.2.9 or quiesce application processing so there are no writes and upgrade all nodes.  If you encounter a similar situation during migration and need help with mitigation please contact our support team.
...
* https://issues.couchbase.com/browse/NCBC-3171[NCBC-3171]: Documents may be written to the wrong location in a mixed-mode cluster setup. See Special Note for more details.
