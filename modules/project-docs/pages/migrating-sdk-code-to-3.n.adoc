...
==== Connecting with SDK 3.9.0 and Later

SDK 3.9.0 introduces explicit authenticator objects and a consolidated TLS settings API.
The recommended approach is to use `WithPasswordAuthentication()` or `WithCertificateAuthentication()` instead of setting properties directly on `ClusterOptions`.
...
`WithCertificateAuthentication()` automatically enables TLS.
Only provide *client* certificates here; server CA certificates, if using `CustomRootTrust`, should go into `TlsSettings.TrustedServerCertificateFactory`.
...
Please see the xref:howtos:sdk-authentication.adoc[Authentication how-to] for the full reference, including certificate rotation, runtime authenticator swapping, and custom server CA trust.
