...
Name: *Orphaned Response Logger*
Cluster Option: `EnableOrphanedResponseLogging(bool)`
+
Default:  `false`
+
Enables the default Orphaned Response Logger.
+
If false, the Orphaned Response Logger will not log orphaned responses if a request fails to complete for some reason.
