...
and to distribute load away from data nodes in MDS topologies.
...
the index _*covers*_ the query, and eliminates the need to fetch the actual values from the Data Service.
...
Creating indexeson buckets with many existing documents can take a long time.
...
include::6.5@server:learn:services-and-indexes/indexes/index-replication.adoc[tag=scan_consistency]
