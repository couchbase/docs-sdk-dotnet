...
For Couchbase Server 6.0 and earlier, the plan is cached by the SDK (up to a limit of 5000). 
On Couchbase Server 6.5 and newer, the plan is stored by the Query service -- up to a limit of 16 000 plans per Query node. 

For Couchbase Server 6.0 and earlier, the N1QL optimizer is completely rule-based, and it will mostly ignore placeholders and constants. 
...
Do not turn off the `AdHoc` flag for _every_ query since only a finite number of query plans (currently 5000) can be stored in the SDK.
