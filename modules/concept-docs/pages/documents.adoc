...
IMPORTANT: Couchbase Server deals with expiry in seconds but the .NET SDK takes expiry values in ms. 
Values are converted to seconds before being sent to the server. 
(You can also use InsertOptions.Expiry(System.TimeSpan) to set a Unix timespan in seconds.)
