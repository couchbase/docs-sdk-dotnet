...
NOTE: Fields like *total_us* give a time that includes not just the operation, but all possible retries, so will often be longer than a single operation such as that given by *last_dispatch_us*.
