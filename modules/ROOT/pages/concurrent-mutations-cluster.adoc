...
[source,csharp]
----
var result = bucket.Get<dynamic>("docid").Value;
----
....
...
|===
| 0.00ms
| (#2): `var new_doc = bucket.Get<dynamic>("docid").Value;`

| 0.00ms
| (#1): `var new_doc = Get<dynamic>("docid").Value;`
...
| 0.01ms
| (#1): `bucket.Replace("docid", new_doc);`

| 0.02ms
| (#2): `bucket.Replace("docid", new_doc);`
|===
...
var result = bucket.Get<dynamic>("docid");
var new_doc = result.Value;
Console.WriteLine(JsonConvert.Serialize(new_doc));
----
....
{"a_field": "a_value"}
....
...
[source,csharp]
----
Console.WriteLine(new_result.Cas);
----
....
195896137937427
....
...
while (true)
{
    var result = bucket.Get<dynamic>("userId")
    var user_doc = result.Value;
    user_doc.email = "new-email";
    var updateResult = bucket.Update("docId", user_doc, result.Cas);
...
The following blocks show how to use [.api]`lock` and [.api]`unlock` with the .NET SDK.

[source,charp]
----
var result = bucket.GetAndLock("key", TimeSpan.FromSeconds(5));
...
else
{
    ...
    var setResult = bucket.Update("key", result.Value, result.Cas);

    // or, you can manually unlock a document
    var unlockResult = bucket.Unlock("key", result.Cas);
}
----
