...
You can also use xref:n1ql-query.adoc[N1QL queries] and xref:full-text-search-overview.adoc[Full Text Search] to access documents by means other than their IDs, however these query operations Couchbase eventually translates into primitive key-value operations, and exist as separate services outside the data store.
...
[source,csharp]
----
bucket.MutateIn<dynamic>("docid").ArrayAddUnique("tags", "fast").Execute();
----
...
[source,csharp]
----
var results = bucket.LookupIn<dynamic>("user:kingarthur").Get("contact.name").Get("contact.email").Execute();
Console.WriteLine("{0} - {1}", results.Content("contact.name"), results.Content("contact.email"));
----
...
Within a single cluster the `Increment()` is atomic, as is `Decrement()`; across XDCR however, if two clients connecting to two different (bidirectional) clusters issue `incr` concurrently, this may (and most likely will) result in the value only getting incremented once in total.
The same is the case for `Decrement()`.

A document may be used as a counter if its value is a simple ASCII number, like `42`.
Couchbase allows you to increment and decrement these values atomically using a special [.api]`Increment` operation.
The example below shows how to use counters:

[source,csharp]
----
var result = bucket.Increment("counter_id", 20, 100);
Console.WriteLine(result.Value); //100L

result = bucket.Increment("counter_id", 1);
Console.WriteLine(result.Value); //101L

result = bucket.Decrement("counter_id", 50);
Console.WriteLine(result.Value); //51L
----

In the above example, a counter is created by using the [.api]`Counter` CSharp method with an [.param]`initial` value.
...
[source,csharp]
----
var result = bucket.Get<dynamic>("counter_id");
if (shouldIncrementCounter)
{
    bucket.Upsert("counter_id", result.Value + incrementAmount, result.Cas);
}
----
...
You will find several ways of working with counters https://docs.couchbase.com/sdk-api/couchbase-net-client-2.7.16/search.html?SearchText=counter[in the API docs].
...
See the xref:datastructures.adoc[data structures documentation] for details on implementation.

Data structures in Couchbase are similar in concept to data structures in, for example, C#:

* *Map* is like C# `Dictionary<K,V>`, and is a key-value structure, where a value is accessed by using a key string.
* *List* is like a C# `List<T>` and is a sequential data structure.
...
[source,csharp]
----
var result = bucket.MapAdd("map_id", "name", "Mike Goldsmith", true);
----
...
[source,csharp]
----
var result = bucket.ListGet<dynamic>("Id", 0).Value;
var result = bucket.MapGet<dynamic>("map_id", "name").Value;
----
...
----
# Store the document
bucket.Upsert("binary_doc", new List<byte>{0x0});

while (true)
{
    var value = bucket.Get<List<byte>>("binary_doc").Value;
    value.Add(0x02);

    var replace = bucket.Replace("binary_doc", value);
    if (replace.Success)
    {
        break;
    }
...
Console.WriteLine(bucket.Get<List<byte>>("binary_doc").Value);
----
...
----
# Store the document
bucket.Insert("binary_doc", new  byte[] {0x01});

# Append a fragment
bucket.Append("binary_doc", new byte[] {0x02});

Console.WriteLine(bucket.Get<byte[]>("binary_doc"));
----
...
* The _GetAndTouch_ operation allows an application to retrieve a document while modifying its expiration time.
...
NOTE: Couchbase Server deals with expiry in seconds but the .NET SDK takes expiry values in ms. 
Values are converted to seconds before being sent to the server. 
A time of zero will set the document to never expire
(a negative number will set expiry to immediate -- creating a xref:6.0@server:learn:buckets-memory-and-storage/storage.adoc#tombstones[tombstone]). 
