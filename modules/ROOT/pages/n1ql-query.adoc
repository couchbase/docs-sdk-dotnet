...
:page-aliases: querying,concept-docs:n1ql-query,prepared-statements
...
Here is an example of a query executed in .NET:

[source,csharp]
----
var query = new QueryRequest("SELECT type FROM `travel-sample` LIMIT 4");
foreach (var row in bucket.Query<dynamic>(query))
{
    Console.WriteLine(JsonConvert.Serialize(row));
}
...
[source,csharp]
----
var query = new QueryRequest("SELECT airportname as n FROM `travel-sample` where type="airport" LIMIT 4");
foreach (var row in bucket.Query<dynamic>(query))
{
    Console.WriteLine(JsonConvert.Serialize(row));
}
...
[source,csharp]
----
var query = new QueryRequest($"SELECT airportname FROM `travel-sample` WHERE city=${city}");
----
...
[source,csharp]
----
var query = new QueryRequest("SELECT airportname FROM `travel-sample` WHERE city=$1")
    .AddPositionalParamter("London");
----
...
[source,csharp]
----
var statement = "SELECT airportname FROM `travel-sample` " +
...
var query = new QueryRequest(statement)
    .AddNamedParameter("country", "United States")
    .AddNamedParameter("altitude", 500)
    .AddNamedParameter("min_lat", -50)
    .AddNamedParameter("max_lat", 50)
    .AddNamedParameter("min_lon", -180)
    .AddNamedParameter("max_lon", 0);
----
...
[source,csharp]
----
var query = new QueryRequest("SELECT airportname FROM `travel-sample` WHERE country=$1")
    .AddPositionalParameter("USA")
    .AdHoc(false);
----
...
CAUTION: *When running an application using Prepared Statements through the .NET SDK* -- if you plan to upgrade Couchbase Server from 6.0.x or earlier to 6.5.0 or later, you will need to xref:6.5@server:install:upgrade-strategy-for-features.adoc#prepared-statements[restart the app or otherwise work around] a change in the Server's behaviour.
